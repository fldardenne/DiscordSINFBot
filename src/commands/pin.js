...
module.exports = {
	...
	async execute(client, interaction) {
		...
		if (!permission) {
			let rv =  interaction.reply({ content: "Insufficient privileges to pin this message. This is a very severe offence. This incident will be reported to the administrator.", ephemeral: true });
			
			fs.appendFileSync("log", `[${new Date()}] ${member.user.tag} attempted to pin a message without sufficient privileges`);
			return rv;
		}
